...
%AV_SUBPLOT Create axes in tiled positions.
%   H = AV_SUBPLOT(m,n,p), or SUBPLOT(mnp), breaks the Figure window
...
elseif(isempty(position))
    if (min(thisPlot) < 1)
        ...
        if exist('flag_tight'), PERC_OFFSET_B=0;PERC_OFFSET_T=0;end
        if nrows > 2
